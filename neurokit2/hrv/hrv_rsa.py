...
def _get_multipeak_window(nperseg, window_number=8):
    ...
    r_num = 2 * C - np.exp(-C * B / 2) * (2 * C * np.cos(np.pi * B * length) - 4 * np.pi * length * np.sin(np.pi * B * length))
...
def _hrv_rsa_formatinput(ecg_signals, rsp_signals, rpeaks=None, sampling_rate=1000):
    ...
    if isinstance(ecg_signals, pd.DataFrame):
        ...
        elif "ECG_R_Peaks" in ecg_signals.columns:
    ...
    if rpeaks is None:
        try:
            ...
            raise ValueError("NeuroKit error: _hrv_rsa_formatinput(): Wrong input, we couldn't extract rpeaks indices.") from e
    else:
